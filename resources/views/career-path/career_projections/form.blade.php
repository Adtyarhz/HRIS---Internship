...
<div class="container-fluid">
    <div class="form-content-container">
        <div class="card-body">
            ...
            @if (isset($careerProjection))
                <form action="{{ route('career-projections.update', [$employee, $careerProjection]) }}" method="POST">
                    @method('PUT')
            @else
                <form action="{{ route('career-projections.store', $employee) }}" method="POST">
            @endif
                @csrf
                <div class="row">
                    <div class="col-12">
                        <label class="page-title">
                            {{ isset($careerProjection) ? 'Edit' : 'Add' }} Career Projection for {{ $employee->full_name }}
                            <hr>
                        </label>

                        {{-- Projected Position --}}
                        <div class="form-group row align-items-center">
                            <label for="projected_position_id" class="col-md-2 col-form-label">
                                Projected Position <span class="text-danger">*</span> :
                            </label>
                            <div class="col-md-3">
                        ...
                        {{-- Timeline --}}
                        <div class="form-group row align-items-center">
                            <label for="timeline" class="col-md-2 col-form-label">Timeline <span class="text-danger">*</span> :</label>
                            <div class="col-md-3">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-4">
                                <textarea name="readiness_notes" id="readiness_notes"
                                    class="form-control @error('readiness_notes') is-invalid @enderror"
                                    rows="6" placeholder="Description of Readiness Notes">{{ old('readiness_notes', $careerProjection->readiness_notes ?? '') }}</textarea>
                ...
                <div class="row mt-4">
                    <div class="col-12">
                        <div class="form-buttons-container">
                            <a href="{{ route('career-projections.index', $employee) }}" class="btn btn-cancel">Cancel</a>
                            <button type="submit" class="btn btn-submit">
                                {{ isset($careerProjection) ? 'Update' : 'Save' }}
                            </button>
                        </div>
                    </div>
                </div>
